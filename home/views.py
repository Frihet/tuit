...
def home(request):
    ...
    keys['widgets'] = [
        ...
        Widget(_('My oldest open tickets'),
               Issue.objects.exclude(current_status = status_closed).filter(assigned_to=request.user)
.order_by('-creation_date'),
        ...
        Widget(_('My last updates'),
               IssueUpdate.objects.order_by('-creation_date').filter(user=request.user).distinct('issue_id'), request, 'my_latest_updates', class_names='full_width'),
